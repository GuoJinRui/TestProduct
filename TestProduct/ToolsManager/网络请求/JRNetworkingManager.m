...
@implementation JRNetworkingManager

static NSMutableArray * _allSessionTask; // 所有会话任务数组
static NSDictionary * _baseParameters; // 设置基础参数
static NSArray * _filtrationCacheKey; // 缓存的key的过滤器
static AFHTTPSessionManager * _sessionManager;
static NSString * const NetworkResponseCache = @"JRNetworkResponseCache";
static NSString * _baseURL; // 设置基础url
...
+ (void)initialize{
    ...
    // 设置服务器返回结果的类型:(AFHTTPResponseSerializer, AFJSONResponseSerializer, AFXMLParserResponseSerializer, AFPropertyListResponseSerializer, AFImageResponseSerializer, )
    /**
     *  默认情况下AFHTTPResponseSerializer为nil
     *  不会对data序列化
     */

    /**
     *  默认情况下AFJSONResponseSerializer支持的MIME类型 ：
     *  `application/json`、`text/json`、`text/javascript`
     */
    
    /**
     *  默认情况下AFXMLParserResponseSerializer支持的MIME类型 ：
     *  `application/xml`、`text/xml`
     */

    /**
     *  默认情况下AFPropertyListResponseSerializer支持的MIME类型 ：
     *  `application/x-plist`
     */
    
    /**
     *  默认情况下AFImageResponseSerializer支持的MIME类型 ：
     *  `image/tiff`、`image/jpeg`、`image/gif`、`image/png`、`image/ico`
     *  `image/x-icon`、`image/bmp`、`image/x-bmp`、`image/x-xbitmap`、`image/x-win-bitmap`
     */

    
    // AFHTTPSessionManager默认情况下是使用AFJSONResponseSerializer的实例
...
{
    [self HTTPWithMethod:JRRequestMethodGET url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)POSTWithURL:(NSString *)url
        ...
        cachePolicy:(JRCachePolicy)cachePolicy
           callback:(JRHttpRequest)callback
{
    [self HTTPWithMethod:JRRequestMethodPOST url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)HEADWithURL:(NSString *)url
        ...
        cachePolicy:(JRCachePolicy)cachePolicy
           callback:(JRHttpRequest)callback
{
    [self HTTPWithMethod:JRRequestMethodHEAD url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PUTWithURL:(NSString *)url
       ...
       cachePolicy:(JRCachePolicy)cachePolicy
          callback:(JRHttpRequest)callback
{
    [self HTTPWithMethod:JRRequestMethodPUT url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PATCHWithURL:(NSString *)url
         ...
         cachePolicy:(JRCachePolicy)cachePolicy
            callback:(JRHttpRequest)callback
{
    [self HTTPWithMethod:JRRequestMethodPATCH url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)DELETEWithURL:(NSString *)url
          ...
          cachePolicy:(JRCachePolicy)cachePolicy
             callback:(JRHttpRequest)callback
{
    [self HTTPWithMethod:JRRequestMethodDELETE url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
              ...
              callback:(JRHttpRequest)callback
{
    ...
    if (cachePolicy == JRCachePolicyIgnoreCache) {
        //只从网络获取数据，且数据不会缓存在本地
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }else if (cachePolicy == JRCachePolicyCacheOnly){
    ...
    }else if (cachePolicy == JRCachePolicyNetworkOnly){
        //先从网络获取数据，同时会在本地缓存数据
    ...
    }else if (cachePolicy == JRCachePolicyCacheElseNetwork){
        //先从缓存读取数据，如果没有再从网络获取
        [self httpCacheForURL:url parameters:parameters withBlock:^(id<NSCoding> object) {
            if (object) {
                callback ? callback(object, nil) : nil;
    ...
    }else if (cachePolicy == JRCachePolicyNetworkElseCache){
    ...
    }else if (cachePolicy == JRCachePolicyCacheThenNetwork){
        ...
        //缓存策略错误，将采取 JRCachePolicyIgnoreCache 策略
//        ATLog(@"缓存策略错误");
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }
...
#pragma mark -- 网络请求处理
+ (void)httpWithMethod:(JRRequestMethod)method url:(NSString *)url parameters:(NSDictionary *)parameters callback:(JRHttpRequest)callback{
...
+(void)dataTaskWithHTTPMethod:(JRRequestMethod)method url:(NSString *)url parameters:(NSDictionary *)parameters
...
#pragma mark -- 上传文件
+ (void)uploadFileWithURL:(NSString *)url parameters:(NSDictionary *)parameters name:(NSString *)name filePath:(NSString *)filePath progress:(JRHttpProgress)progress callback:(JRHttpRequest)callback
{
...
#pragma mark -- 上传图片文件
+ (void)uploadImageURL:(NSString *)url parameters:(NSDictionary *)parameters images:(NSArray<UIImage *> *)images name:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType progress:(JRHttpProgress)progress callback:(JRHttpRequest)callback;
{
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩-添加-上传图片
...
#pragma mark -- 下载文件
+(void)downloadWithURL:(NSString *)url fileDir:(NSString *)fileDir progress:(JRHttpProgress)progress callback:(JRHttpDownload)callback
{
...
+ (NSString *)getMethodStr:(JRRequestMethod)method{
    switch (method) {
        case JRRequestMethodGET:
            return @"GET";
            break;
        case JRRequestMethodPOST:
            return @"POST";
            break;
        case JRRequestMethodHEAD:
            return @"HEAD";
            break;
        case JRRequestMethodPUT:
            return @"PUT";
            break;
        case JRRequestMethodPATCH:
            return @"PATCH";
            break;
        case JRRequestMethodDELETE:
            return @"DELETE";
            break;
